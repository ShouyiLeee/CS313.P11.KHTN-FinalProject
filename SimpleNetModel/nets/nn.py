...
class Conv(torch.nn.Module):
    def __init__(self, in_channels, out_channels, activation, k=1, s=1):
        super().__init__()
        p = (k - 1) // 2
        self.conv = torch.nn.Conv2d(in_channels, out_channels, k, s, p, bias=False)
...
class Residual(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, s=1):
        ...
        self.add_m = s != 1 or in_channels != self.expansion * out_channels # Determine if a shortcut connection is needed

        self.conv1 = Conv(in_channels, out_channels, torch.nn.ReLU(True), 3, s)
        self.conv2 = Conv(out_channels, out_channels, torch.nn.Identity(), 3, 1)

        if self.add_m:
            self.conv3 = Conv(in_channels, out_channels, torch.nn.Identity(), s=s)
...
class Bottleneck(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, s=1):
        ...
        self.conv1 = Conv(in_channels, out_channels, torch.nn.ReLU(True))
        self.conv2 = Conv(out_channels, out_channels, torch.nn.ReLU(True), 3, s)
        self.conv3 = Conv(out_channels, out_channels * self.expansion, torch.nn.Identity())

        if self.add_m:
            self.conv4 = Conv(in_channels, self.expansion * out_channels, torch.nn.Identity(), s=s)
...
class Backbone(torch.nn.Module):
    def __init__(self, device):
        ...
        for handle in self.backbone.hook_handles:
            handle.remove() # remove to avoid conflict
        self.outputs = {}
...
class Discriminator(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.fc = torch.nn.Sequential(torch.nn.Linear(in_channels, out_channels),
                                      torch.nn.BatchNorm1d(out_channels),
                                      torch.nn.SiLU(inplace=True),
                                      torch.nn.Linear(out_channels, 1, False))
        for m in self.modules():
            if isinstance(m, torch.nn.Linear):
                torch.nn.init.xavier_normal_(m.weight)
