...
data_dir = 'D:\IT\GITHUB\CS313.P11.KHTN-FinalProject\SimpleNet\metal_nut'
...
def load_backbone():
    ...
    # backbone = nn.resnet50()
    # state_dict = torch.load('./weights/resnet50.pth')['state_dict']
...
def train(args):
    util.setup_seed()
    util.configure_multiprocessing()
    # Model
    model = load_backbone()
    ...
    with open('weights/step.csv', 'w') as f:
        ...
        for epoch in range(args.epochs):
            ...
            training_losses.append(m_loss.avg)  # Store training loss for this epoch
            
            # Save models and evaluate
            saved_model_g  = copy.deepcopy(model_g)
            saved_model_d  = copy.deepcopy(model_d)
            last = test(args, device, saved_model_g , saved_model_d )
            ...
            # Save models
            checkpoint = {'model_g': saved_model_g, 'model_d': saved_model_d}
    ...
    plt.legend()
    plt.savefig('weights/training_loss_plot.png')
    plt.show()
    ...
    for samples, targets, filenames in tqdm.tqdm(loader, desc):
        ...
        with torch.no_grad():
            ...
            if args.test:
                ...
                masks = util.generate_anomaly_mask(args, patch_scores, device)
